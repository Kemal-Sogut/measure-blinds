...
/**
 * Unit tests for the authoritative line-item adjustment arithmetic.
 *
 * This module is a TWIN of apps/web/src/lib/lineItemAdjustments.ts, and
 * apps/web/src/lib/lineItemAdjustments.test.ts asserts the identical
 * cases. A divergence here is a browser quoting a price the Worker will
 * not charge, which is exactly the failure AI_GUIDELINES rule 1 exists
 * to prevent — so the two suites must be edited together.
 */
