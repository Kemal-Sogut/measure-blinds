 ...
 *
 * This module is a PURE function of its inputs and is the AUTHORITATIVE
 * twin of `apps/web/src/lib/priceLock.ts` (the live preview must show
 * the frozen price the save is about to keep). The two, and their
 * mirrored `priceLock.test.ts` suites, MUST stay in sync. Database
 * access lives in `lib/priceLockStore.ts`, never here.
 */
