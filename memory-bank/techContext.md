...
| pdf-lib | ^1.17.1 | Server-side PDF generation (pure JS; @react-pdf needs WASM, forbidden in workerd) |
| Vitest | ^3.2.0 | Unit + route-level tests in BOTH workspaces (`pnpm test` / `pnpm --filter api test`) |

## Dependencies
| Service | Purpose | Status |
|---------|---------|--------|
| Supabase | Database + Auth | LIVE: `lgbxxlwsdeuhdgzrjjen` — all migrations applied |
| Cloudflare | Workers hosting | LIVE (verified 2026-07-07): `blinds-nisa-api` (API) + `measure-blinds` (web assets), both on workers.dev |
| Resend.com | Transactional email | Key configured in `.dev.vars` (`RESEND_FROM` orders@blindsnisa.com); verify live delivery |
