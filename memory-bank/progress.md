...
**Works:** `apps/web/public/manifest.webmanifest` (`display: "standalone"`, so a launch from the
home-screen icon has no address bar or tabs), the real brand mark at
`apps/web/public/logo.svg`, and five derived icons generated by `scripts/generate-icons.mjs`
(`favicon.svg`, `icon-192.png`, `icon-512.png`, `icon-maskable-512.png`,
`apple-touch-icon.png`). `index.html` links the manifest and carries the Apple tags Safari
needs in place of it. The purple placeholder favicon is gone.

**Verified:** web build clean with all six assets in `dist/`; manifest 200 as
`application/manifest+json`, every icon 200, no console errors; `pnpm check` clean; web tests
98/10 pass.

**Owed:**
- ⬜ **Install it on a real phone and a real tablet** — the one thing no local check covers.
  Confirm the icon is the brand mark (not a screenshot of the page), the launch has no browser
  chrome, and the iOS status bar does not overlap the shell header.
- ⬜ **Merge to `main`** — that is what deploys it. Nothing is live before then.
- ⬜ Audit in-app back navigation for standalone mode: Android drops the browser back button,
  so a route with no in-app way back becomes a dead end once installed.
- ⬜ Decide on offline. There is no service worker, so launching needs network. Non-trivial:
  pricing is server-authoritative, so offline writes need a queue and a re-price-on-sync rule.
